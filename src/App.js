            ...
            if (j == 0) {
              if (j != res[k].length - 1 && j != res[k].length - 2)
                outputs[counter] = {
                  name: "M" + counter,
                  drain: "N" + counter,
                  gate: res[k][j + 1] == "`" ? res[k][j] : "~" + res[k][j],
                  source: "vdd",
                  body: "vdd",
                  type: "pmos",
                };
              ...
              outputs[counter] = {
                ...
                drain: variable[i].output,
                gate: res[k][j + 1] == "`" ? res[k][j] : "~" + res[k][j],
                source: outputs[counter - 1].drain,
                body: outputs[counter - 1].drain,
                type: "pmos",
              };
              counter++;
            } else {
          ...
          if (
            ...
            if (c == 0 && c == res.length - 2) {
              outputs[counter] = {
                name: "M" + counter,
                drain: variable[i].output,
                gate: res[c][p + 1] == "`" ? "~" + res[c][p] : res[c][p],
                source: 0,
                body: 0,
                type: "nmos",
              };
              ...
              if (c == 0) var D = variable[i].output;
              else var D = blockS[c - 1];
              // var D = outputs[counter - 1].source;

              var S;
              if (c == res.length - 2) S = 0;
              else {
                S = "N" + counter;
                blockS[c] = S;
              }
